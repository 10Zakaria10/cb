...
async function _post<TData, TBody>(
  path: string,
  config?: ClientRequestPayloadConfig<TBody>,
): Promise<TData> {
  const { data } = await instance.post<TData>(path, config?.data);
  return data;
}

async function _delete<TData>(
  path: string,
  config?: ClientRequestConfig,
): Promise<TData> {
...
async function _put<TData, TBody>(
  path: string,
  config?: ClientRequestPayloadConfig<TBody>,
): Promise<TData> {
...
async function _patch<TData, TBody>(
  path: string,
  config?: ClientRequestPayloadConfig<TBody>,
): Promise<TData> {
